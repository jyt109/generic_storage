...
from .._google_drive_storage import GoogleDriveStorage


@pytest.fixture(scope="module")
def google_drive() -> GoogleDriveStorage:
    environ = os.environ
    settings_env = "GDRIVE_SETTING_PATH"
    cred_env = "GDRIVE_CREDNENTIAL_PATH"
    if settings_env not in environ or cred_env not in environ:
        raise Exception(f"Set {settings_env} and {cred_env} env variables")
    settings_path = os.environ[settings_env]
    cred_path = os.environ[cred_env]
